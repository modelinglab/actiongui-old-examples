 ...
 */
package org.modelinglab.external.authentication.vm;
...
import org.modelinglab.external.authentication.MyAGSession;
...
/**
 * This is the servlet that runs the AG Application.
 * 
 * This class is a very similar to a normal AGServlet but it provides an AGSession that manages
 * the callerID. The other difference is that this servlet is not responding to the /* pattern
 * (which means all resources in this war) but /app/* insted. So to access this application you
 * need to write this /app/ part.
 * <p/>
 * As you can see in the source code, the servlet is also respondig to /VAADIN/*. This is because
 * Vaadin requires it, as you can see <a href="https://vaadin.com/book/-/page/application.environment.html">
 * here</a/>
 * 
 * 
 */
@WebServlet(value = {"/app/*", "/VAADIN/*"}, name = "AG-Servlet")
...
public class MyAGServlet extends AGServlet {
    ...
    private static class MyResourceProvider extends AppImpl.ResourceProvider {
        ...
        protected AGSession createSession(WrappedSession session) {
            return new MyAGSession(session);
        }
